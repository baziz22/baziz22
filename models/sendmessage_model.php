...
class Sendmessage_Model extends Model{
    ...
    public function send_message($fields){
        ...
        if($stmt->execute()){
          ...
          header("Location: ../" . $where_at ."?message_success=$message_success&wrong=0");
          
        }else{
          $message_success = 'message-not-sent';
          header("Location: ../" . $where_at ."?message_success=$message_success&wrong=1");
