...
class Register_Model extends Model{
    ...
    function is_username_available($name){
        ...
        $stmt->bindValue(':login-user-name', $name);
        $stmt->execute();
        $data = $stmt->fetch();
        $count = $stmt->rowCount();
        if ($count > 0){
    ...
    public function is_email_available($email){
        $sql = "SELECT * FROM users WHERE user_email = :login-email";
        $stmt = $this->db->connect()->prepare($sql);
        // Bind value: to bind our search string to the placeholder by referring to its position in the SQL query.
        //$stmt->bindParam('user_name',$name) ;
        $stmt->bindValue(':login-email', $email);
        $stmt->execute();
        $data = $stmt->fetch();
        $count = $stmt->rowCount();
        if ($count > 0){
