...
class User_Model extends Model{
    ...
    public function edit($data) {
      pre_r($data);
      $sql = 'UPDATE users SET user_name = :username, user_password = :password, user_role = :the_role WHERE user_name = :username';
      $stmt = $this->db->connect()->prepare($sql);
      $stmt->execute(array(
    ...
    public function activate_user($id){
        $sql = 'UPDATE FROM users SET user_status = 1 WHERE user_id = :id';
        $stmt = $this->db->connect()->prepare($sql);
        //$stmt->bindValue($id);
