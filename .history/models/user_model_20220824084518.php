...
class User_Model extends Model{
    ...
    public function create($data){
        ...
        $sql = "INSERT INTO `users` (user_name, user_password, user_role) VALUES (:username, :password, :role)";
        $stmt = $this->db->connect()->prepare($sql);
    ...
    public function editSave($data) {
        pre_r($data);
        $sql = 'UPDATE users SET user_name = :username, user_password = :password, role = :the_role WHERE user_id = :id';
        $stmt = $this->db->connect()->prepare($sql);
        $stmt->execute(array(
            ':id' => $data['user_id'],
