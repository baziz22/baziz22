...
class Login extends Controller{
    ...
    public function login_run(){
        ...
        if ($request_method === 'GET') {
            ...
            $hashed_password = $this->model->create
            // if($check_hashed_password == true){
                $this->loadModel('login');
                //$this->model->login_user($username); 
                $this->model->check_login_username($username, $password); 
            /* } else {
                $error_message = "Username or password not correcty";
                header('Location: ../login?error-message=$error_message');
            } */
