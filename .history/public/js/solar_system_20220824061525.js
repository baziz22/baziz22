...
const p_radii = [12, 20, 30, 40, 50];
let p_radians = new Array(5).fill(0);
const p_velocities = [0.434, 1.174, 1, 0.802, 1.12];//, 0.323, 0.228, 0.182];
...
//const circle_solar = document.querySelector(".circle1");

p_orbits.forEach((p_orbit, index) => {
  p_orbit.style.height = `${p_radii[index]}vmin`;
  p_orbit.style.width = `${p_radii[index]}vmin`;
  /* console.log(`${p_radii[index]}`);
  console.log(p_radii[p_radii.length-1]);
  circle_solar.style.height = `${p_radii[p_radii.length-1]}`;
  circle_solar.style.width = `${p_radii[p_radii.length-1]}`;
