        ...
        <div class="dashboard-main">
          ...
          <div class="dashboard-main-upper-middle">
            ...
            <div class="dashboard-statistics">
              ...
              <div class="dashboard-edit-user">
                ...
                ?>
                <form action="../editSave/<?php echo $this->singleUser['user_id']; ?>" method="POST">
                  <label for=""></label><input type="text" name="username" placeholder="username" value="<?php echo $this->singleUser['user_name']; ?>"><br />
                  <label for=""></label><input type="text" name="password" placeholder="Password"><br />
                  <label for=""></label>
                  <select name="the_role" id="">
                    <option value="default" <?php if ($this->singleUser['user_role'] == 'default') echo 'selected'; ?>>Default</option>
                    <option value="user" <?php if ($this->singleUser['user_role'] == 'user') echo 'selected'; ?>>User</option>
                    <option value="super" <?php if ($this->singleUser['user_role'] == 'super') echo 'selected'; ?>>Super</option>
                    <option value="admin" <?php if ($this->singleUser['user_role'] == 'admin') echo 'selected'; ?>>Admin</option>
                    <!-- <option value="owner" <?php if ($this->singleUser['user_role'] == 'owner') echo 'selected'; ?>>Owner</option> -->
                  </select><br />
          ...
          <div class="dashboard-main-lower-middle">
            ...
            <div class="user-list-table">
              <table>
                ...
                //foreach ($this->model->db->pagination($start_from, $per_page_record) as $key => $value) {
                  $paginate = new user();
                  $pages_number = $paginate->pagination($paginate, $per_page_record);
                  echo $start_from.'<br>';
                  foreach ($paginate->pagination($paginate, $per_page_record) as $key => $value) {
                  if($key+1 == $start_from){
                    echo $key.'<br>';
                    echo $start_from.'<br>';
                    break;
                  ...
                  echo '<tr>';
                  echo '<td>' .$key. '|'. $value['user_id'] . '. </td>';
                  echo '<td>' .$key. '|'. $value['user_name'] . '</td>';
                  echo '<td>' .$key. '|'. $value['user_role'] . '</td>';
                  echo '<td>' .$key. '|'. $value['user_status'] . '</td>';
            ...
            <div class="inline-block">
              <input style="display:inline" id="page" type="number" min="1" max="<?php echo $total_pages ?>" placeholder="<?php echo $page . "/" . $total_pages; ?>" required>
              <button onClick="go2Page();" class="btn dashbord-btn" style="display:inline-block">Go</button>
            </div>
