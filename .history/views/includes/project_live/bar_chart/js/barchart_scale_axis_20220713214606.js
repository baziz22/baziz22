...
.select(".project-live-modal-body")
.append("svg")
.attr("id", "barchart-svg")
.attr("width", barchart_scale_axis_width)
.attr("height", barchart_scale_axis_height)
...
d3.csv("<?php URLROOT; ?>views/includes/project_live/bar_chart/data/XYZ.csv").then((d) => {
    dataset = d;
...
function createBarChart(){
        ...
        // For sorting - not completely done yet
        d3.select("#sort-checkbox")
        ...
        function toggleSort() {
            ...
            // store all values in an array to evaluate them later
            let array_value = dataset.map((data) => data.value);
            let array_years = dataset.map((data) => data.year);
            console.log("This is array value: " + array_value );
            barchartXscale.domain(array_years);
            d3.selectAll(".bar")
            .attr("x", function(d) {
