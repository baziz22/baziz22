...
let pie_chart_csv_label = d3
  .arc()
  .outerRadius(pie_chart_csv_radius )
  .innerRadius(pie_chart_csv_radius -30);
...
.text(function(d) { 
    if (d.population != 0) { 
        if (d.data.population.length == 10) {
            return d.data.continent + " " + parseFloat(d.data.population/1e9).toFixed(2) + " B" +"" 
        } else {
            return d.data.continent + " " + parseFloat(d.data.population/1e6).toFixed(2) + " M" +"" 
        }
  ...
  pie_chart_csv_arc
    ...
    .attr("transform", function (d) {
      ...
      console.log("centroid: ",pie_chart_csv_label.centroid(d[0],d[1]));
      return "translate(" + pie_chart_csv_label.centroid(d) + ")";
      
      console.log("centroid: ",pie_chart_csv_label.centroid(d[0]));
...
pie_chart_csv_svg
  ...
  .attr(
    "transform",
    "translate(" + (pie_chart_csv_width / 2 - 100) + "," + -20 + ")"
  )
  .append("text")
