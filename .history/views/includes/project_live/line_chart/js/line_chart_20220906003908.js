...
let line_graph_svg = d3
  ...
  /* let g_line_chart_x_axis = g_main_line_chart
  .append("g")
  .attr("id", "g_line_chart_x_axis");   
  let g_line_chart_y_axis = g_main_line_chart
  .append("g")
  .attr("id", "g_line_chart_y_axis");  */
// parse the date / time
var parseTime = d3.timeParse("%d-%b-%y");
var formatTime = d3.timeFormat("%e %B");

// set the ranges
let line_graph_x_scale = d3.scaleTime().range([0, line_graph_width- 120] );
let line_graph_y_scale = d3.scaleLinear().range([line_graph_height- 100, 0]);
...
d3.csv("views/includes/project_live/line_chart/data/date_data.csv").then((data) => {
  ...
  line_graph_svg
    .append("g")
    .attr("transform", "translate(0," + (line_graph_height-100) + ")")
  ...
  line_graph_svg
    ...
    .on("mouseover", function (event, d) {
      this.transition().duration(200).style("opacity", 0.9)
        .html("Bader Binsunbil")
        .style("left", event.pageX - 120 + "px")
        .style("top", event.pageY - 140 + "px");
    });
    // Grid Lines
    const yGridlines = d3.axisLeft()
    .scale(line_graph_y_scale)
    .ticks(11)
    .tickSize(-line_graph_width+120,0,0)
    .tickFormat('')

    line_graph_svg.append('g')
    .call(yGridlines)
